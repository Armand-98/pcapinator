...
Signals that read the letters (bigram fit, pronounceability) carry weight in
proportion to how much of the name is letters, and the weight they give up
passes to the signals that read shape. That is what catches hex-style names:
"1c4e1517b560e945d8" has too few letters to say anything about its bigrams, so
the verdict rests on its digits, entropy and length instead.
...
On the labelled set in tests/test_dga.py (177 real domains against 200 names
from five generator styles) this scores 0.88 accuracy at 0.77 recall with no
false positives, and the worst benign name lands 0.10 below the threshold.
...
Benign patterns deliberately tested against (tests/test_dga.py):
  ...
  - reverse lookups, mDNS service names, .local and bare single labels, which
    are skipped
...
Known false-positive and false-negative classes:
  ...
  - names of four characters or fewer are unreliable in both directions and are
    damped hard enough that they are effectively never reported.
...
from .bigrams import INDEX, LOG_PROB

VOWELS = frozenset("aeiouy")
MAX_LABEL = 63          # a DNS label cannot exceed this; hostile names can
...
WEIGHT_BIGRAM = 0.62
WEIGHT_PRONOUNCE = 0.13
WEIGHT_ENTROPY = 0.09
WEIGHT_LENGTH = 0.03
WEIGHT_DIGIT = 0.13
LETTER_WEIGHTS = WEIGHT_BIGRAM + WEIGHT_PRONOUNCE
...
# signal agrees, and never on one signal alone.
...
class DgaScore:
    ...
    def describe(self) -> str:
        return (f"{self.domain} score {self.score:.2f} "
                f"(bigram {self.bigram:.2f}/t, entropy {self.entropy:.2f}b, "
...
def score_domain(name: str) -> DgaScore | None:
    ...
    confidence = CONFIDENCE_FLOOR + (1.0 - CONFIDENCE_FLOOR) * _ramp(
        length, CONFIDENCE_SHORT, CONFIDENCE_FULL)

    return DgaScore(
        domain=domain, label=label, length=length,
        bigram=bigram, entropy=entropy, vowel_ratio=vowel_ratio,
        consonant_run=consonant_run, digit_ratio=digit_ratio,
        bigram_score=bigram_score, pronounce_score=pronounce_score,
...
def _combine(letter_share: float, letter_parts: dict[float, float],
             shape_parts: dict[float, float]) -> float:
    ...
    shape_base = sum(shape_parts)

    total = sum(weight * letter_share * value
                for weight, value in letter_parts.items())
    total += sum(weight / shape_base * shape_weight * value
                 for weight, value in shape_parts.items())
    return total
